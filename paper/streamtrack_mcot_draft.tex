...
We verify tracking accuracy by confirming output parity with the batch baseline~\cite{wang2024overlap}: our system produces identical global identity assignments for the same input data, ensuring that all downstream accuracy metrics (IDF1, HOTA) are preserved by construction (visually affirmed in Figure~\ref{fig:visual_comparison}). We focus our quantitative evaluation on systems-level metrics: throughput (windows per second), peak processing latency for the clustering stage, and similarity matrix generation time.
...
Future directions include: (i) multi-hop topology reasoning, where cameras beyond immediate neighbors contribute to association via transitive similarity; (ii) learning topology weights from historical data to dynamically adjust join selectivity; and (iii) a full runtime-reconfigurable topology where aggregation queries are dynamically rewritten in response to \texttt{CameraTopologyTable} updates.
