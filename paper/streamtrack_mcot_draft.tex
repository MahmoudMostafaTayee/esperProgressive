...
\begin{table}[h]
    \centering
    \caption{Stage 1: Architectural Speedup (\textbf{Theirs} vs. \textbf{Ours-Base}) for Scene 1.}
...
\subsection{Stage 2: Topological Optimization Enhancement (Scene 2)}
...
Table~\ref{tab:topology_perf} shows the benchmarking results for Scene 2. Transitioning to the relational streaming model (\textbf{Ours-Base}) yields a 2.2$\times$ speedup over the batch baseline ($\sim$210ms vs. 95ms). The subsequent introduction of topological pruning (\textbf{Ours-Opt}) yields an additional \textbf{21.1$\times$} optimization, resulting in a cumulative \textbf{46.7$\times$ speedup} relative to the original tracking pipeline.

\begin{table}[h]
    \centering
    \caption{Stage 2: Impact of Topological Optimization on MCPT Stage (Scene 2).}
...
% [REVIEW] TODO: Fill with selected datasets and synthetic topology-change scenarios.
