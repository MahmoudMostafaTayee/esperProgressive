...
\begin{itemize}[leftmargin=*]
    ...
    \item A performance-optimized implementation (\textbf{Ours}) that achieves a \textbf{19.4$\times$ speedup in similarity matrix generation} and a 4.7$\times$ speedup in global ID assignment compared to the conventional offline baseline (\textbf{Theirs}).
\end{itemize}
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}lrrr@{}}
        ...
        Representative Selection$_a$ & 13.62 & 362.72 & 0.03$\times$ \\
        Similarity Matrix Generation & 1260.31 & 64.93 & \textbf{19.41$\times$} \\
        Similarity Matrix Zeroing & 253.24 & 207.17 & 1.22$\times$ \\
        Similarity Replacement & $<$0.01 & $<$0.01 & 1.00$\times$ \\
        Hierarchical Clustering (HC) & 112.68 & 704.00 & 0.16$\times$ \\
        Global ID Assignment & 52.10 & 10.88 & \textbf{4.79$\times$} \\ \midrule
        \textbf{Total MCPT Time} & \textbf{2023.73} & \textbf{1795.52} & \textbf{1.13$\times$} \\ \bottomrule
        \multicolumn{4}{l}{\footnotesize $a$: The baseline (\textbf{Theirs}) utilizes a local cache for representative nodes in this run.}
    \end{tabular}
\end{table}

\subsection{Performance Analysis}
The results highlight a significant breakthrough in \emph{Similarity Matrix Generation}, where \textbf{Ours} achieves a 19.41$\times$ speedup. This is attributed to the efficient handling of primitive arrays and JIT-optimized execution of cosine similarity loops. Conversely, the Hierarchical Clustering (HC) stage currently exhibits higher latency in our implementation compared to the baseline's C-backed implementations. This identifies a clear optimization target for future work, potentially through the integration of more efficient native clustering libraries. Overall, the system achieves a processing time of 1.79 seconds for a 4-camera window, demonstrating its potential for real-time operation.
