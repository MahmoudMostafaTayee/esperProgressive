...
Once per-camera synchronized events trigger the association phase, the native MCPT module performs representative selection based on quality criteria: (i) the presence of high-confidence anatomical keypoints (facial features, limb joints) and (ii) the stability of the appearance embedding vector relative to the tracklet centroid. This logic ranks detections within a window and retains only the most discriminative visual data, forwarding it to the subsequent Re-ID clustering block.
