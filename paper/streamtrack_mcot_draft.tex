...
\begin{itemize}[leftmargin=*]
    ...
    \item A performance-optimized implementation (\textbf{Ours}) that achieves a \textbf{46.6$\times$ cumulative speedup} in the MCPT clustering stage relative to the batch baseline (\textbf{Theirs}). This is realized through a two-stage enhancement: (1) a migration to a \textbf{relational streams-and-tables architecture} that yields a 4.28$\times$ architectural speedup, and (2) \textbf{Topology-Aware Stream Join} optimization that provides a further 21.1$\times$ gain.
\end{itemize}
...
Table~\ref{tab:topology_perf} shows the benchmarking results for the AI City Challenge 2025 Track 1 dataset~\cite{aicity2025track1}. Transitioning to the relational streaming model (\textbf{Ours-Base}) yields a 2.2$\times$ speedup over the batch baseline ($\sim$210.4ms vs. 95.2ms). The subsequent introduction of topological pruning (\textbf{Ours-Opt}) yields an additional \textbf{21.1$\times$} optimization, resulting in a cumulative \textbf{46.6$\times$ speedup} relative to the original tracking pipeline.
...
\begin{table}[h]
    ...
    \label{tab:topology_perf}
    \begin{tabular}{@{}lrrrr@{}}
        \toprule
        \textbf{Metric} & \textbf{Theirs (ms)} & \textbf{Ours-Base (ms)} & \textbf{Ours-Opt (ms)} & \textbf{Cum. Speedup} \\ \midrule
        Avg. MCPT Stage Time & $\sim$210.40 & 95.19 & \textbf{4.50} & \textbf{46.7$\times$} \\
        Max MCPT Stage Time & -- & 122.00 & \textbf{6.00} & -- \\
        Total Window Latency & Offline & 225.00 & \textbf{100.00} & -- \\ \bottomrule
    \end{tabular}
\end{table}

\subsection{Overall Performance Analysis}
The results confirm that \textbf{STREAMTRACK} achieves a state-of-the-art balance between latency and accuracy. The transition to a \textbf{relational streams-and-tables architecture} (\textbf{Ours-Base}) removes the significant bottlenecks associated with similarity matrix generation (as seen in Section 7.1) by leveraging incremental windowing and continuous state management. Furthermore, the use of reconfigurable topology tables (\textbf{Ours-Opt}) ensures the system remains scalable as the camera network grows, keeping processing latency well within the constraints for high-frequency video analytics.
