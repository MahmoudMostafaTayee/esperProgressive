...
\begin{itemize}[leftmargin=*]
    ...
    \item A performance-optimized implementation (\textbf{Ours}) that achieves up to a \textbf{210$\times$ speedup} in the primary similarity generation stage and a \textbf{32$\times$ overall speedup} relative to the batch baseline (\textbf{Theirs}). This is realized through a two-stage enhancement: (1) a migration to a \textbf{relational streams-and-tables architecture} that yields an architectural speedup in coordinate projection and state maintenance, and (2) \textbf{Topology-Aware Stream Join} optimization that delivers an \textbf{8.2$\times$} latency reduction per association call when scaling to 9 cameras.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Architectural Speedup (\textbf{Theirs} vs. \textbf{Ours-Base}) for Woven WTS and NVIDIA SmartSpaces Datasets (Total Latency over a 50-window stress test).}
...
\begin{figure}[H]
    ...
    \caption{Topological partitioning in the NVIDIA SmartSpaces network. The figure illustrates how cameras with overlapping fields of view form localized clusters that are joined as independent stream topologies, while spatially distant cameras are excluded from comparison. Grouping cameras that share no physical overlap eliminates unnecessary algorithmic complexity from the association stage.}
...
For the NVIDIA SmartSpaces dataset, our system achieved a \textbf{32.4$\times$} average speedup in total MCPT latency, with the core similarity matrix generation being over \textbf{210$\times$} faster than the baseline. Most significantly, while the baseline's execution time begins to deviate sharply as the tracklet density grows, our relational streaming architecture maintains a sub-second processing pace for the entire 50-window run. This confirms the system's ability to maintain high-velocity tracking in large-scale multi-camera environments where batch-oriented approaches fail to meet real-time constraints.
...
The breakthrough in \emph{Similarity Matrix Generation} (up to 210$\times$ speedup on the NVIDIA SmartSpaces dataset) is specifically attributed to the architectural shift from batch-oriented list processing to \textbf{stream-oriented relational operations} which enable efficient memory locality and incremental updates. Overall, the system achieves a processing time of 516 milliseconds for a 4-camera window in the Woven WTS dataset, demonstrating its robust capability for real-time operation in diverse multi-camera environments.
...
We presented \textbf{STREAMTRACK}, a relational streams-and-tables architecture for multi-camera object tracking. By recasting MCOT as a continuous query problem and utilizing topology-aware stream joins, we achieved up to a \textbf{210$\times$ speedup} in similarity generation and a \textbf{32$\times$ overall speedup} over batch-oriented baselines on the NVIDIA SmartSpaces dataset. Scaling to a 9-camera network with topology-aware partitioning yields a further \textbf{8.2$\times$} latency reduction per association call. Our system ensures real-time performance even as the camera network grows, while providing an architecture that supports unique operational benefits like runtime reconfigurability and SQL-based inspectability.
