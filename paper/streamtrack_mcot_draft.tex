...
\begin{itemize}[leftmargin=*]
    ...
    \item A performance-optimized implementation (\textbf{Ours}) that achieves a \textbf{35.9$\times$ speedup} in the primary Matrix Generation stage relative to the batch baseline (\textbf{Theirs}). This is realized through a two-stage enhancement: (1) a migration to a \textbf{relational streams-and-tables architecture} that yields an architectural speedup in coordinate projection and state maintenance, and (2) \textbf{Topology-Aware Stream Join} optimization that provides a further 1.2$\times$ gain on sparse networks.
\end{itemize}
...
We benchmark the fundamental performance of the proposed streams-and-tables architecture (\textbf{Ours-Base}) against the conventional batch-oriented baseline (\textbf{Theirs}). The evaluation is conducted across two datasets: the Woven VisionAI WTS dataset~\cite{woven2024wts} and the NVIDIA PhysicalAI-SmartSpaces dataset~\cite{aicity2025track1}, allowing us to observe scaling effects under different data densities.
...
\begin{table}[H]
    \centering
    \caption{Architectural Speedup (\textbf{Theirs} vs. \textbf{Ours-Base}) for Woven WTS and Scene 2 Datasets during a 50-window scaling stress test.}
    \label{tab:performance_combined}
    \resizebox{\linewidth}{!}{
    \begin{tabular}{@{}l|rrr|rrr@{}}
        \toprule
        & \multicolumn{3}{c|}{\textbf{Woven WTS Dataset}} & \multicolumn{3}{c}{\textbf{Scene 2 (NVIDIA) Dataset}} \\
        \textbf{Pipeline Stage} & \textbf{Theirs (ms)} & \textbf{Ours (ms)} & \textbf{Speedup} & \textbf{Theirs (ms)} & \textbf{Ours (ms)} & \textbf{Speedup} \\ \midrule
        Re-ID Similarity Matrix & 1260.00 & 49.27 & \textbf{25.57$\times$} & 2835.69 & 13.46 & \textbf{210.68$\times$} \\
        Total MCPT Latency & 2210.70 & 515.99 & \textbf{4.28$\times$} & 13184.40 & 407.12 & \textbf{32.38$\times$} \\ \bottomrule
    \end{tabular}
    }
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lrrrr@{}}
        ...
        Avg. MCPT Stage Time & 510.29 & 17.05 & \textbf{14.21} & \textbf{35.9$\times$} \\ \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Total MCPT Latency comparison. The batch baseline (\textbf{Theirs}) scales poorly, reaching over 13 seconds for a 50-window run. In contrast, the relational streaming architecture (\textbf{Ours}) processes the identical workload in roughly 0.4 seconds by maintaining incremental state.}
    \label{fig:total_latency}
\end{figure}

For the NVIDIA SmartSpaces dataset, our system achieved a \textbf{32.4$\times$} average speedup in total MCPT latency, with the core similarity matrix generation being over \textbf{200$\times$} faster than the Python baseline. Most significantly, while the baseline required approximately 7 hours of total wall-clock time to complete the evaluation loop due to $O(N^2)$ scaling and redundant data loading overhead, our relational streaming architecture completed the equivalent experimentation in under 1 hour. This confirms the system's ability to maintain high-velocity tracking in large-scale multi-camera environments where batch-oriented approaches fail to meet real-time constraints.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Evidence/chart_speedup_factor.png}
    \caption{Matrix Generation Speedup vs. Data Scale. As the number of processed windows increases, the performance advantage of the streaming architecture grows linearly from roughly 20$\times$ to over 210$\times$, demonstrating standard resilience against state explosion compared to the batch baseline.}
