...
\begin{table}[H]
    \centering
    \caption{Architectural Speedup (\textbf{Theirs} vs. \textbf{Ours-Base}) for Woven WTS and Scene 2 Datasets (Total Latency over a 50-window stress test).}
...
\begin{table}[H]
    \centering
    \caption{Stage 2: Impact of Topological Optimization on MCPT Stage (Average Latency per Window, NVIDIA SmartSpaces).}
    \label{tab:performance_nvidia}
    \resizebox{\linewidth}{!}{
    \begin{tabular}{@{}lrrrr@{}}
        \toprule
        \textbf{Metric} & \textbf{Theirs (ms)} & \textbf{Ours-Base (ms)} & \textbf{Ours-Opt (ms)} & \textbf{Cum. Speedup} \\ \midrule
        Avg. MCPT Stage Time & 263.69 & 8.14 & \textbf{7.92} & \textbf{33.3$\times$} \\ \bottomrule
    \end{tabular}
    }
\end{table}

\subsection{Scalability Analysis: 9-Camera Stress Test}
To evaluate the system's performance under increased topological complexity, we expanded the test environment to include 9 cameras (01, 11, 13, 17, 19, 21, 25, 27, 30) from the NVIDIA SmartSpaces dataset. We compare the \textbf{Global} (single-group) execution against the \textbf{Topology-Aware} (4-group) partitioning. 

\begin{table}[H]
    \centering
    \caption{9-Camera Performance Comparison: Global vs. Topology-Aware.}
    ...
    \begin{tabular}{@{}lccc@{}}
        ...
        Avg. MCPT Latency (per call) & 156.78 ms & 19.13 ms & \textbf{8.2$\times$} \\
        Total Window Latency (Summed) & 156.78 ms & 44.90 ms & \textbf{3.5$\times$} \\
        Max Group Latency (Parallel) & 156.78 ms & 16.37 ms & \textbf{9.6$\times$} \\ \bottomrule
    \end{tabular}
    }
\end{table}

The results show that partitioning the 9-camera network into four topological groups reduces the sequential processing latency by \textbf{3.5$\times$}. When considering the potential for parallel execution of these independent groups, the effective latency is reduced by \textbf{9.6$\times$}. This performance gain is primarily driven by the reduction in clustering complexity from $O(N^3)$ to $O(n^3)$, where $n$ is the number of tracklets per local group.
...
For the NVIDIA SmartSpaces dataset, our system achieved a \textbf{32.4$\times$} average speedup in total MCPT latency, with the core similarity matrix generation being over \textbf{210$\times$} faster than the Python baseline. Most significantly, while the baseline's execution time begins to deviate sharply as the tracklet density $N$ grows, our relational streaming architecture maintains a sub-second processing pace for the entire 50-window run. This confirms the system's ability to maintain high-velocity tracking in large-scale multi-camera environments where batch-oriented approaches fail to meet real-time constraints.
