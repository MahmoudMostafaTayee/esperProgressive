...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lccc@{}}
        ...
        Avg. MCPT Latency & 156.78 ms & 19.13 ms & \textbf{8.2$\times$} \\ \bottomrule
    \end{tabular}
    }
\end{table}

The results show that partitioning the 9-camera network into four topological groups reduces the average MCPT clustering latency by \textbf{8.2$\times$}, from 156.78 ms to 19.13 ms per association call. This performance gain is primarily driven by reducing the number of tracklets processed per clustering call, localizing the workload to each topological neighborhood.
