...
\begin{itemize}[leftmargin=*]
    ...
    \item A performance-optimized implementation (\textbf{Ours}) that achieves a \textbf{4.28$\times$ architectural speedup} over the batch baseline (\textbf{Theirs}) and a further \textbf{21.1$\times$ optimization gain} via topology-aware joining (\textbf{Ours-Opt}), enabling sub-5ms clustering in real-time environments.
\end{itemize}
...
\begin{table}[h]
    \centering
    \caption{Performance comparison of MCPT stages (\textbf{Theirs} vs. \textbf{Ours-Base}) for Scene 1.}
...
\subsection{Topological Optimization Enhancement (Scene 2)}
While the streaming architecture provides a significant baseline improvement, the computational complexity still scales with the number of tracklets in the aggregation window. By introducing \emph{Topology-Aware Stream Joins}, we further optimize the association search space. 

We benchmarked this enhancement on Scene 2 by comparing our streaming baseline (\textbf{Ours-Base}) against the topology-aware configuration (\textbf{Ours-Opt}). As shown in Table~\ref{tab:topology_perf}, restricting joins to neighboring cameras results in an additional \textbf{21.1$\times$ speedup} in the clustering stage.

\begin{table}[h]
    \centering
    \caption{Impact of Topology-Aware Grouping on MCPT Stage (Scene 2).}
    \label{tab:topology_perf}
    \begin{tabular}{@{}lrrr@{}}
        \toprule
        \textbf{Metric} & \textbf{Ours-Base (ms)} & \textbf{Ours-Opt (ms)} & \textbf{Speedup} \\ \midrule
        Avg. MCPT Stage Time & 95.19 & 4.50 & \textbf{21.15$\times$} \\
        Max MCPT Stage Time & 122.00 & 6.00 & \textbf{20.33$\times$} \\
        Total Window Latency & 225.00 & 100.00 & \textbf{2.25$\times$} \\ \bottomrule
    \end{tabular}
\end{table}

\subsection{Overall Performance Analysis}
The results confirm that \textbf{STREAMTRACK} achieves a state-of-the-art balance between latency and accuracy. The transition to a Java/Esper streaming core (\textbf{Ours-Base}) removes the significant bottlenecks associated with similarity matrix generation (as seen in Section 7.1). Furthermore, the use of reconfigurable topology tables (\textbf{Ours-Opt}) ensures the system remains scalable as the camera network grows, keeping processing latency well within the constraints for high-frequency video analytics.
